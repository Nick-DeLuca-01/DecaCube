...
void Scene_CubeLeft::sEnemyFaceChange(sf::Time dt)
{
	for (auto e : _enemyData.enemyManager.getEntities()) {
...
void Scene_CubeLeft::sEnemyBehaviour()
{
	for (auto e : _enemyData.enemyManager.getEntities()) {
...
void Scene_CubeLeft::flipper(std::shared_ptr<Entity> entity)
{
	enemyAwareMovement(entity);
}

void Scene_CubeLeft::gunner(std::shared_ptr<Entity> entity)
{
...
void Scene_CubeLeft::stalker(std::shared_ptr<Entity> entity)
{
	auto& tfm = entity->getComponent<CTransform>();
	auto& sight = entity->getComponent<CSight>();
	bool seesPlayer = canSeePlayer(entity);
...
void Scene_CubeLeft::charger(std::shared_ptr<Entity> entity)
{
...
void Scene_CubeLeft::revenant(std::shared_ptr<Entity> entity)
{
	auto& sight = entity->getComponent<CSight>();
...
Vec2 Scene_CubeLeft::pickBestNode(std::vector<Vec2> availableNodes)
{
...
void Scene_CubeLeft::enemyChargeMovement(std::shared_ptr<Entity> enemy)
{
	std::vector<Vec2> availableNodes;
...
void Scene_CubeLeft::enemySmoothMovement(std::shared_ptr<Entity> enemy)
{
	auto& tfm = enemy->getComponent<CTransform>();
	auto pPos = _player->getComponent<CTransform>().pos;
...
void Scene_CubeLeft::findIntersection(std::shared_ptr<Entity> enemy)
{
	auto& pathFinding = enemy->getComponent<CPathFinding>();
	auto& tfm = enemy->getComponent<CTransform>();
...
bool Scene_CubeLeft::touchingPlayer(std::shared_ptr<Entity> entity)
{
	auto overlap = Physics::getOverlap(entity, _player);
	return (overlap.x > 0 && overlap.y > 0);
}

void Scene_CubeLeft::clearBullets()
{
	for (auto bullet : _entityManager.getEntities("bullet")) {
...
Vec2 Scene_CubeLeft::midPixelToGrid(float midX, float midY, std::shared_ptr<Entity> entity)
{
...
bool Scene_CubeLeft::canMoveInDirection(std::string direction, std::shared_ptr<Entity> entity)
{
...
sPtrEntt Scene_CubeLeft::getCurrentTile(std::shared_ptr<Entity> entity)
{
...
int Scene_CubeLeft::changeFace(int currentFace, bool isFlipper)
{
...
bool Scene_CubeLeft::alreadyTraveled(std::vector<Vec2> visitedNodes, Vec2 targetNode)
{
