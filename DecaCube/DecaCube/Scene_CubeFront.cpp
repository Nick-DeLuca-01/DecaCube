...
void Scene_CubeFront::gunner(std::shared_ptr<Entity> entity)
{
...
void Scene_CubeFront::sunAndMoon(std::shared_ptr<Entity> entity)
{
...
void Scene_CubeFront::defender(std::shared_ptr<Entity> entity)
{
	auto& tfm = entity->getComponent<CTransform>();
...
void Scene_CubeFront::stalker(std::shared_ptr<Entity> entity)
{
	auto& tfm = entity->getComponent<CTransform>();
	auto& sight = entity->getComponent<CSight>();
	bool seesPlayer = canSeePlayer(entity);
...
void Scene_CubeFront::charger(std::shared_ptr<Entity> entity)
{
...
void Scene_CubeFront::revenant(std::shared_ptr<Entity> entity)
{
	auto& sight = entity->getComponent<CSight>();
...
Vec2 Scene_CubeFront::pickBestNode(std::vector<Vec2> availableNodes, Vec2 target)
{
	float minDistance = 1000.f;
	Vec2 closestNode;
...
Vec2 Scene_CubeFront::pickRandomNode(std::vector<Vec2> availableNodes)
{
...
void Scene_CubeFront::enemyUnawareMovement(std::shared_ptr<Entity> enemy)
{
	auto& tfm = enemy->getComponent<CTransform>();
...
void Scene_CubeFront::enemyDefenceMovement(std::shared_ptr<Entity> enemy, Vec2 itemLocation)
{
	auto& tfm = enemy->getComponent<CTransform>();
	auto& pathFinding = enemy->getComponent<CPathFinding>();
	std::vector<Vec2> availableNodes;
...
void Scene_CubeFront::enemyChargeMovement(std::shared_ptr<Entity> enemy)
{
	std::vector<Vec2> availableNodes;
...
void Scene_CubeFront::enemySmoothMovement(std::shared_ptr<Entity> enemy)
{
	auto& tfm = enemy->getComponent<CTransform>();
	auto pPos = _player->getComponent<CTransform>().pos;
...
void Scene_CubeFront::findIntersection(std::shared_ptr<Entity> enemy)
{
	auto& pathFinding = enemy->getComponent<CPathFinding>();
	auto& tfm = enemy->getComponent<CTransform>();
...
void Scene_CubeFront::enemyMovement(Vec2 distance, std::shared_ptr<Entity> enemy)
{
	auto& pathFinding = enemy->getComponent<CPathFinding>();
	auto& tfm = enemy->getComponent<CTransform>();
...
bool Scene_CubeFront::canSeePlayer(std::shared_ptr<Entity> enemy)
{
	auto& tfm = enemy->getComponent<CTransform>();
...
bool Scene_CubeFront::touchingPlayer(std::shared_ptr<Entity> entity)
{
	auto overlap = Physics::getOverlap(entity, _player);
	return (overlap.x > 0 && overlap.y > 0);
}

void Scene_CubeFront::clearBullets()
{
	for (auto bullet : _entityManager.getEntities("bullet")) {
...
void Scene_CubeFront::killPlayer()
{
...
int Scene_CubeFront::changeFace(int currentFace, bool knowsPlayerPos)
{
